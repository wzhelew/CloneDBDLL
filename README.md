...
Библиотека (.NET Framework 4.8 класическа DLL) за клониране на MySQL/MariaDB база данни (минимум версия 5.x). Тя използва `CloneService.CloneDatabase` за копиране на структури, данни, изгледи, тригери и процедури между източник и дестинация.
...
> При MariaDB 5.5.6 на Windows bulk SQL клониране/възстановяване може да хвърля грешки заради `DEFINER=` в `SHOW CREATE VIEW/TRIGGER/ROUTINE`; библиотеката автоматично премахва `DEFINER=` от дефинициите преди създаване, за да се прилага текущият потребител и да се избегнат сривове на dump/import.
